function UltimateScrollingTable(tabId, height, width, sortId) {
    ...
    */
    ...
    if (tFootMC !== undefined) {
        ...
        iy.removeAttribute('id');
        if (sortId && divHR !== undefined) { /* zap all the thFR th ids so there is only one copy in thHR */
            if (divHR !== undefined) {
    ...
    function calculate() { /* calculate window width & height, width & height to use,
    ...
    */
        ...
        if (height <= 0) {
            ...
            if (windowHeight - UST.top >= 100 + headerHeight + footerHeight + scrollHeight) { /* Heuristic: if there are at least 100px left on the screen, for the
    ...
    if (sortId && divHR !== undefined) { /* Add an onClick handler to the header rows in thHR that clicks the
