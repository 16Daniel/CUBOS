...
namespace waCubos
{
    public class bin
    {
        ...
        public DataSet HojaCalculoTotales(string fecha, string grupo,string empresa)
        {  
            try
            {
                ParameterCollection Coll = new ParameterCollection();
                ...
                if (empresa.Equals("BD1") && grupo.Equals("WA"))
                {
                    this.dbc = new Database(Settings.Default.Cadena.Replace("%bd%", "BD2"), Settings.Default.TimeOut);
                    return this.dbc.loadDataSet("was_HojaCalculoTotalesQRO", Coll); 
                }
                if (empresa.Equals("BD2") && grupo.Equals("WA"))
                {
                   return this.dbc.loadDataSet("was_HojaCalculoTotalesMX", Coll);
                }
                   return this.dbc.loadDataSet("was_HojaCalculoTotales2", Coll);
        ...
        public DataSet DetallesDelivery(string fecha, string grupo,string empresa)
        {
            try
            {
                ...
                {
                    this.dbc = new Database(Settings.Default.Cadena.Replace("%bd%", "BD2"), Settings.Default.TimeOut);
                    return this.dbc.loadDataSet("was_DetallesDeliveryQRO", Coll);
                }
                if (empresa.Equals("BD2") && grupo.Equals("WA"))
                {
                    return this.dbc.loadDataSet("was_DetallesDeliveryMX", Coll);
                }
                    return this.dbc.loadDataSet("was_DetallesDelivery2", Coll);
            }
            catch (Exception ex)
            {
                throw new Exception("was_DetallesDelivery: " + ex.Message);
            }
        }
